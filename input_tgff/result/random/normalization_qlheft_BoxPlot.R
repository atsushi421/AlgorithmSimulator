## read data file
d1 <- read.table("./QLHEFT/0-7.txt")
d1_nume <- as.numeric(d1$V1)
d1_median <- median(d1_nume)
d1 <- d1 / d1_median  # QLHEFT_0-7_median
d2 <- read.table("./QLHEFT/8-15.txt")
d2_nume <- as.numeric(d2$V1)
d2_median <- median(d2_nume)
d2 <- d2 / d2_median  # QLHEFT_8-15_median
d3 <- read.table("./QLHEFT/16-23.txt")
d3_nume <- as.numeric(d3$V1)
d3_median <- median(d3_nume)
d3 <- d3 / d3_median  # QLHEFT_16-23_median
d4 <- read.table("./QLHEFT/24-31.txt")
d4_nume <- as.numeric(d4$V1)
d4_median <- median(d4_nume)
d4 <- d4 / d4_median  # QLHEFT_24-31_median
d5 <- read.table("./QLHEFT/32-39.txt")
d5_nume <- as.numeric(d5$V1)
d5_median <- median(d5_nume)
d5 <- d5 / d5_median  # QLHEFT_32-39_median
qlheft <- cbind(d1, d2, d3, d4, d5) # bind data

## read data file
d1 <- read.table("./Propose/0-7.txt")
d1 <- d1 / d1_median  # QLHEFT_0-7_median
d2 <- read.table("./Propose/8-15.txt")
d2 <- d2 / d2_median  # QLHEFT_8-15_median
d3 <- read.table("./Propose/16-23.txt")
d3 <- d3 / d3_median  # QLHEFT_16-23_median
d4 <- read.table("./Propose/24-31.txt")
d4 <- d4 / d4_median  # QLHEFT_24-31_median
d5 <- read.table("./Propose/32-39.txt")
d5 <- d5 / d5_median  # QLHEFT_32-39_median
propose <- cbind(d1, d2, d3, d4, d5) # bind data

## read data file
d1 <- read.table("./HEFT/0-7.txt")
d1 <- d1 / d1_median  # QLHEFT_0-7_median
d2 <- read.table("./HEFT/8-15.txt")
d2 <- d2 / d2_median  # QLHEFT_8-15_median
d3 <- read.table("./HEFT/16-23.txt")
d3 <- d3 / d3_median  # QLHEFT_16-23_median
d4 <- read.table("./HEFT/24-31.txt")
d4 <- d4 / d4_median  # QLHEFT_24-31_median
d5 <- read.table("./HEFT/32-39.txt")
d5 <- d5 / d5_median  # QLHEFT_32-39_median
...
xaxis_scale <- c("0-7", "8-15", "16-23", "24-31", "32-39")
